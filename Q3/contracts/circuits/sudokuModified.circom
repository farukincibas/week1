...
template sudoku() {
    ...
    //[assignment] hint: you will need to initialize your RangeProof components here
        ...
        component firstComponent[9][9];
        component secondComponent[9][9];
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
           firstComponent[i][j] = RangeProof(32);
           secondComponent[i][j]= RangeProof(32);
           
            firstComponent[i][j].range[0] <== 0;
            firstComponent[i][j].range[1] <== 9;
            firstComponent[i][j].in <== puzzle[i][j];

            secondComponent[i][j].range[0] <== 0;
            secondComponent[i][j].range[1] <== 9;
            secondComponent[i][j].in <== solution[i][j];
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
              secondComponent[i][j].out === firstComponent[i][j].out;
        }
    }
